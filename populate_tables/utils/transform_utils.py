...
def format_columns(
    season_to_date_team_df: pd.DataFrame
) -> pd.DataFrame:
    season_to_date_team_df = season_to_date_team_df.drop(
        'PLAYER_NAME',
        axis=1
    )
    ptsCol = season_to_date_team_df.pop('PTS')
    season_to_date_team_df.insert(5, 'Pts', ptsCol)
    season_to_date_team_df.rename(
        columns={
            'GAME_ID': 'GameID',
            'TEAM_ID': 'TeamID',
            'PLAYER_ID': 'PlayerID',
...
def format_team_boxscores(
    df: pd.DataFrame
) -> pd.DataFrame:
    df = df.drop(
        ['TEAM_ID', 'SEASON_ID', 'TEAM_ABBREVIATION', 'TEAM_NAME'],
        axis=1
    )
    df.rename(
        columns={
...
def format_player_totals(
    ...
    df.rename(
        columns={
            ...
            'PTS': 'Pts',
            'PLUS_MINUS': 'PlusMinus',
            'GP_RANK': 'GamesPlayedRank',
            'W_RANK': 'WinsRank',
            'L_RANK': 'LossesRank',
            'W_PCT_RANK': 'WinPctRank',
            'MIN_RANK': 'MinRank',
            'FGM_RANK': 'FGMRank',
            'FGA_RANK': 'FGARank',
